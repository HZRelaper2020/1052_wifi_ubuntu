 ...
 * Main Scan app
 */
//static uint8_t internalbuf[1024*10];
//static nons_buffer_init_t g_wwdbuf;
...
void scan_main( void )
{
 ...
 //   g_wwdbuf.internal_buffer = internalbuf;
  //  g_wwdbuf.buff_size = sizeof(internalbuf);

    //wwd_buffer_init(&g_wwdbuf);
    wwd_buffer_init(NULL);

    result = wwd_management_wifi_on( 0x5541 );
    ...
    while (1)
    {
        ...
        /* 开始扫描 */
        WPRINT_APP_INFO(("Starting Scan2\n"));
