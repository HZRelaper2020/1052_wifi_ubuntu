                          ...
                          uint32_t 			 *response)
{
    ...
    SDMMCHOST_COMMAND command = {0U};
    SDMMCHOST_DATA data = {0U};
    uint32_t i = SDIO_RETRY_TIMES*10;
...
//		if (blockSize > 0){
//		PRINTF("cmd:%d,arg:0x%x,blksize:%d\n",cmd,argument,blockSize);
//	
//			for (int tt=0;tt<blockSize;tt++){
//				PRINTF("[%d]0x%x ",tt,txData[i]);
//			}
//			PRINTF("\n");
//		}
    if (dataSize)
    ...
    if (card->cccrflags & kSDIO_CCCRSupportHighSpeed)
    {
        do
        {
            ...
            if ((temp & 0x03U) == 0x03U)
            {
                ...
                card->busClock_Hz =
                    SDMMCHOST_SET_CARD_CLOCK(card->host.base, card->host.sourceClock_Hz, SD_CLOCK_25MHZ);
