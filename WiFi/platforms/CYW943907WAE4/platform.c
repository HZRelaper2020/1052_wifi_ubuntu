...
/** @file
 * Defines board support package for BCM943907WAE4 board
 */
#include "stdio.h"
#include "string.h"
#include "platform.h"
...
/* Gerenic SPI interface. Exposed to the applications through include/wiced_platform.h */
const wiced_spi_device_t wiced_spi_fram =
{
...
void platform_powersave_wakeup_handler( void* arg )
{
    ...
    /* if coming out of SLEEP, disable powersave after waking up,
     * powersave should be enabled back by app when it gets notification
     * from BT that it is ok to sleep.
     *
     * The same check is not needed for DEEP SLEEP as, wakeup from deep-sleep
     * results in warm boot, and powersave is disabled on reboot till application
     * enables it.
     */
...
platform_result_t platform_powersave_sleep_handler( void )
{
    ...
    {
        printf("Failed to request WICED_BT_PIN_HOST_WAKE to wake up from Deep sleep, ignore this message "
                "if Bluetooth powersave is not enabled\n");
        return PLATFORM_ERROR;
    }

    ( platform_mcu_powersave_get_mode( ) == PLATFORM_MCU_POWERSAVE_MODE_DEEP_SLEEP ) ? ( str = deepsleep_string ) : ( str = sleep_string );

    platform_mcu_powersave_bluetooth_sleep_handler( wiced_bt_control_pins[ WICED_BT_PIN_DEVICE_WAKE ] );
...
static void platform_sleep_event_handler( wiced_sleep_event_type_t event, int handler )
{
    ...
    /* Platform powersave sleep/wakeup handlers ensure BT chip can sleep and wakeup
     * APPs processor, the handlers are not needed if BT chip is not being used.
     */
#ifdef USE_WICED_HCI
    switch ( event )
    {
        ...
        case WICED_SLEEP_EVENT_CANCEL:
            platform_powersave_wakeup_handler( NULL );
            break;
