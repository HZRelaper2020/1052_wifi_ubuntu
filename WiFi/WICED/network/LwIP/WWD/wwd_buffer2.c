...
{
    if ( p->type_internal == PBUF_POOL_RX )
    {
...
{
    wiced_assert("Error: Invalid buffer\n", buffer != NULL);
    PRINTF("0x%08x 0x%08x tlen:%d len:%d add_remove:%d  ",*buffer,(*buffer)->payload,(*buffer)->tot_len,(*buffer)->len,add_remove_amount);
