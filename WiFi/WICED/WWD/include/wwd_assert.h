...
#define WICED_ASSERTION_FAIL_ACTION() assert(0)

#ifdef DEBUG
    #ifdef WPRINT_ENABLE_ERROR
        #define WPRINT_ERROR(args)                      do { WPRINT_MACRO(args); WICED_ASSERTION_FAIL_ACTION(); } while(0)
        #define wiced_assert( error_string, assertion ) do { if (!(assertion)) { WPRINT_MACRO((error_string));WICED_ASSERTION_FAIL_ACTION(); } } while(0)
